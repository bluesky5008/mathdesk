...
- 다음 단계 또는 워크플로우: 계획된 구현 종료(사이클 2 승인). TASK-43 재개는 wf-implement, 새 요구는 wf-design
...
- 다음 행동: **계획된 구현 없음 — 사이클 2 승인 완료.** 재개 후보는 ① TASK-43(사용자가 Anthropic API 키를 `.env`에 넣으면 진행, compose에 `MATHDESK_LLM_*`·`ANTHROPIC_API_KEY` 전달 추가 필요) ② [미완료 항목](#미완료-항목)의 외부 조건 검증(문자 실발송·OMR 실스캔·한글 실파일, 준비 문서 `ops/messaging-setup.md`) ③ 기능 공백 후속 후보. 새 기능·변경 요청은 wf-design(DCR)으로 받는다. 실제 학생 데이터를 넣기 전에 테스트 운영 자격 증명 교체가 선행돼야 한다
