...
- 진행 중인 작업: [TASK-49 마스터 데이터 수정 기능 보완](../../plan.md#task-49-마스터-데이터-수정-기능-보완) — TASK-50·51 완료, TASK-52 남음
- 마지막 완료 작업: [TASK-51 반 수정·활성 여부](../../plan.md#task-51-반-수정활성-여부-api-보완-포함) (2026-09-24 01:20)
...
- TASK-52(수강 배정·해제), TASK-24~TASK-39·TASK-43(사이클 2)
...
- 학생·반 관리 화면의 육안 확인 미수행 — 수정 대화상자와 목록 토글은 vitest·빌드·반응형 측정만 거쳤다
...
- 다음 작업: [TASK-52 수강 배정·해제 화면](../../plan.md#task-52-수강-배정해제-화면). 그 뒤 사이클 2 기능 재개([TASK-24](../../plan.md#task-24-m5-성적-통계))
- 사용자가 지정한 순서(2026-09-24): 모바일(완료) → 마스터 데이터 수정(TASK-50·51 완료) → 사이클 2 재개
- 먼저 확인할 사항: [계획 트리](../../plan.md#계획-트리)의 현재 상태, `git status`가 깨끗한지, `docker compose ps`로 개발 스택 기동 여부
- 필요한 문서: [FR-08](../../requirements.md#기능-요구사항), [DES-04](../../design.md#컴포넌트와-책임), [TASK-52 정의](../../plan.md#task-52-수강-배정해제-화면)
...
- **이 작업의 핵심 제약**
  ...
  - TASK-50·51은 완료했다. 두 화면이 쓴 방식(행별 `수정` 대화상자 + 전체 필드 PATCH + 목록 토글)을 TASK-52에도 그대로 쓰면 된다. **`PATCH`는 전치환이라 폼이 다루지 않는 필드는 지워진다.**
...
- 다음 단계 또는 워크플로우: wf-implement 구현 — TASK-52부터
- 시작 조건: 충족됨 — 기준선 `v6` 승인(2026-09-23), TASK-50·51 완료, `git status` 깨끗
...
- 완료된 항목: 기준선 v1~v6 승인, ADR-001~012, DCR-001~005, 사이클 1 전체(TASK-01~TASK-22), TASK-23, TASK-40~TASK-42, 시각 설계 TASK-44~TASK-48, 모바일 TASK-53, TASK-50·51 — 작업 53건 중 33건
- 미완료 항목: TASK-52(수강 배정·해제), TASK-24~TASK-39·TASK-43(사이클 2)
- 차단 요인: 없음
- 다음 행동: **TASK-52 수강 배정·해제 화면.** 선행 테스트(Red)는 "배정 후 해제하면 현재 명단에서 빠지되 해제 이전 날짜의 일일 기록은 그대로 재현된다". **해제는 `DELETE`가 아니라 기간 종료(`end_date`)로 처리한다** — FR-08이 배정 기간 이력 보존을 요구하고, [`daily.py`의 `_enrolled_students`](../../../apps/api/src/mathdesk/daily.py)가 `start_date`/`end_date` 범위로 그날의 명단을 재현하기 때문이다. 기존 `DELETE /classes/{id}/enrollments/{id}`는 오등록 취소 용도로 남긴다. **`PATCH /classes/{id}/enrollments/{id}`가 아직 없으므로 API부터 봐야 한다**(`EnrollmentIn`에 `end_date`는 이미 있다). 화면은 반 관리의 반 행에서 명단을 여는 형태가 자연스럽다
