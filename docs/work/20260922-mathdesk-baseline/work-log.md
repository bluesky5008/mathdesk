...
- 기능 공백(요구사항 충족 범위 밖의 후속 후보)
  ...
  - ~~반 시간표 편집 UI 없음(시드로만)~~ → FR-07 미구현으로 재분류, [DCR-007](./DCR-007-시작-시각만-쓰는-반-시간표.md)에서 처리
...
- 다음 행동: **[DCR-007](./DCR-007-시작-시각만-쓰는-반-시간표.md) 사용자 승인 응답 처리.** 승인이면 FR-07 명확화·AC-37·데이터 모델·REST를 `v8`로 갱신하고 구현 작업(시간표 편집 UI + `end_time` NULL 허용 마이그레이션)을 계획에 넣어 진행한다. 그 뒤 사이클 3 대기(클라이언트 피드백 TASK-54, 계정 준비 TASK-55)
