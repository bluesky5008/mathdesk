...
- 다음 행동: [TASK-45 리포트 카드 HTML 렌더러 전환](../../plan.md#task-45-리포트-카드-html-렌더러-전환) — 기존 `test_report_image.py`의 계약을 새 렌더러로 유지하는 Red부터 시작한다. **모바일 지원 범위(NFR-14) 재검토가 논의 중이며, 진행하기로 하면 DCR-005가 선행한다.**
