...
- ② [DCR-006: 기록이 없는 학생과 반의 삭제](./DCR-006-기록-없는-학생과-반의-삭제.md)를 작성해 승인 요청(`awaiting-approval`). 조사에서 **일일 입력 화면이 조회만으로 빈 수업 세션을 만든다**는 점을 반영해, 반의 "기록"은 입력 내용이 있는 세션으로 정의했다. 원래 TASK-56에 붙여 둔 "DCR-006" 이름은 이 DCR이 먼저 쓰게 되어 "운영 배포처 DCR(번호는 작성 시 발행)"로 바꿨다.
...
- 다음 행동: **[DCR-006](./DCR-006-기록-없는-학생과-반의-삭제.md) 사용자 승인 응답 처리.** 승인이면 요구사항(FR-05 상세·FR-07·AC-36)·설계(DES-04·REST)를 `v7`로 갱신하고 구현 작업을 계획에 넣어 진행한다. 그다음 [TASK-61 클라이언트 안내문](../../plan.md#task-61-클라이언트-안내문-작성)(사용자 지시로 다음 작업)
