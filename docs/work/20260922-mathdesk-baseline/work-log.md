...
- 기능 공백(요구사항 충족 범위 밖의 후속 후보)
  - 수강 해제 종료일은 화면에서 항상 오늘(과거 날짜는 API만) → 배정 시작일과 함께 [TASK-71](../../plan.md#task-71-반-배정-시작일-선택-fr-08)에서 검토
...
- 다음 작업: ~~TASK-68·69~~(완료 2026-09-25), TASK-70(배포됨, 사용자 육안 확인 대기) → [TASK-71 반 배정 시작일 선택](../../plan.md#task-71-반-배정-시작일-선택-fr-08)
- 사용자가 정한 순서(2026-09-25): 권장 순서 3→1→4→2(= 위 순서)

## 인계

- 다음 단계 또는 워크플로우: 사이클 3 — 테스트 운영 피드백 반영(wf-implement 경량 경로: TASK-71, TASK-68·69 완료, TASK-70 육안 확인 대기). 요구사항·권한·데이터가 바뀌면 wf-design(DCR). 배포처 결정은 wf-design
- 시작 조건: TASK-71은 바로 착수 가능(모두 승인된 요구사항 안의 화면 작업, DCR 불필요 확인). TASK-43 재개는 Anthropic API 키가 `.env`에 있을 때
...
- 완료된 항목: 기준선 v1~v12, ADR-001~012, DCR-001~011, 사이클 1·2 전체(TASK-43 보류 제외), 사이클 3의 TASK-61~69(안내문, 학생·반 삭제, 반 시간표, 계정·비밀번호, 담당 강사, 주간 시간표, 강사의 담당 반 수정, 학생/반 탭 분리, 빈 반 안내), TASK-70 구현·배포(육안 확인 대기), 수업 길이 기본 240분(DCR-010), 로그인 유지 버그 수정(2026-09-25)
- 미완료 항목: **TASK-71(다음 착수)**, TASK-70 사용자 육안 확인, TASK-43(보류), 사이클 3 이관 준비 TASK-54~60(클라이언트 승인 대기)
...
- 테스트 운영: **작업이 끝날 때마다 바로 배포한다**(사용자 지시 2026-09-24). 최신 앱 변경(TASK-70 출결 잠금 알림 2026-09-25)까지 배포됨
- 차단 요인: TASK-43은 API 키 대기(사용자 보류). 외부 조건 미검증 항목은 [미완료 항목](#미완료-항목)
- 다음 행동: **[TASK-71 반 배정 시작일 선택](../../plan.md#task-71-반-배정-시작일-선택-fr-08) 착수**(TDD). TASK-70은 사용자가 육안 확인하면 `completed`로. 각 TASK 끝나면 커밋·push·테스트 운영 배포
- 사용자 결정 대기(제안만 함, 2026-09-25): ① 학생 목록에 "반" 칸(현재 배정 반 이름 — 탭 분리 뒤 학생 소속 확인용) ② 명단 다중 배정·반 옮기기 한 동작(테스트 운영 학생 47명이 모두 옛 비활성 반에 있음). 결정되면 TASK-71 뒤에 붙인다
