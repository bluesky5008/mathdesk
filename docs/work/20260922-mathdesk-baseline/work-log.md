...
- 다음 작업: 사용자 확인 대기 — TASK-73(다중 배정·반 옮기기, 사용자가 확인 중). 완료: TASK-68~72. 이후는 사용자가 줄 추가 항목
- 사용자가 정한 순서(2026-09-25): 권장 순서 3→1→4→2(= 위 순서)

## 인계

- 다음 단계 또는 워크플로우: 사이클 3 — 테스트 운영 피드백 반영(wf-implement 경량 경로: TASK-68~72 완료, TASK-73 사용자 확인 대기). 요구사항·권한·데이터가 바뀌면 wf-design(DCR). 배포처 결정은 wf-design
...
- 완료된 항목: 기준선 v1~v12, ADR-001~012, DCR-001~011, 사이클 1·2 전체(TASK-43 보류 제외), 사이클 3의 TASK-61~72(안내문, 학생·반 삭제, 반 시간표, 계정·비밀번호, 담당 강사, 주간 시간표, 강사의 담당 반 수정, 학생/반 탭 분리, 빈 반 안내, 출결 잠금 알림, 배정 시작일, 학생 목록 반 칸), 수업 길이 기본 240분(DCR-010), 로그인 유지 버그 수정(2026-09-25)
...
- 테스트 운영: **작업이 끝날 때마다 바로 배포한다**(사용자 지시 2026-09-24). 최신 앱 변경(TASK-73 다중 배정·반 옮기기 2026-09-25)까지 배포됨
- 차단 요인: TASK-43은 API 키 대기(사용자 보류). 외부 조건 미검증 항목은 [미완료 항목](#미완료-항목)
- 다음 행동: **사용자의 TASK-73 확인 결과와 대시보드 링크 구현 결정(2026-09-25 검토 기록)을 받는다.** 확인되면 `completed`(완료 시점 기입), 문제가 나오면 재현 테스트부터 고친다. 사용자가 새 항목을 주면 계획에 넣고 TDD로 진행. 각 TASK 끝나면 커밋·push·테스트 운영 배포
