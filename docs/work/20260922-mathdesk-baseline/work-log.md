...
### 2026-09-24 — TASK-65 반 담당 강사 지정 완료 (FR-07)

- 선행 테스트: API [`test_class_teacher.py`](../../../apps/api/tests/test_class_teacher.py) 4건 — 같은 캠퍼스 강사 지정 후 그 강사의 반 목록에 보임, 다른 캠퍼스 원장·같은 캠퍼스 원장·없는 ID·사용 안 함 강사 422, 나중에 사용 안 함이 된 기존 담당은 반 수정 시 유지, 수정으로 다른 캠퍼스 사용자 지정 422. 2건 Red(201·200 — 검증 없음 확인). 웹 `ClassesPage.test.tsx` 4건 추가 — 선택지(지정 안 함·사용 중인 강사만), 등록 본문 `teacher_id`, 목록의 강사 이름과 수정 본문 `teacher_id: null`, 사용 안 함 기존 담당 유지, 강사 로그인 시 선택 없음·`/users` 미호출. 3건 Red.
- 내 실수 1건: 목록 이름 확인을 `/김강사/`로 찾아 선택지의 같은 글자와 겹쳤다. 목록 문자열 전체로 찾게 고쳤다.
- 구현 결정(기준선 안): 담당은 "강사" 역할만(계획 목표의 "캠퍼스의 활성 강사 목록"). 원장이 직접 가르치는 반이 필요하면 원장 계정은 이미 모든 반을 보므로 지정이 필요 없다. 422 메시지는 다른 캠퍼스 정보를 드러내지 않는다. 같은 변경에서 `auth.py`·`users.py`의 폐기 예정 상수 `HTTP_422_UNPROCESSABLE_ENTITY`를 저장소 관례(`_CONTENT`)로 맞췄다.
- 검증: API 222건 통과(1 skip, 기존), 웹 94건, `npm run build`, VER-35 통과, 1280px 스크린샷(반 등록 폼의 담당 강사 칸, 목록 `고3 윤A · 고2 · 강사01`).

### 2026-09-24 — TASK-66 주간 시간표 완료 (DCR-008, FR-42)

- 선행 테스트: API [`test_timetable.py`](../../../apps/api/tests/test_timetable.py) 7건 — 원장은 모든 활성 반+강사 이름·요일→시각 순·`end_time` 계산, 강사는 담당 반만(미등록 담당 반은 `unscheduled`), 다른 캠퍼스 반 없음, 미등록·비활성 처리, 수업 길이 90분으로 바꾸면 끝 시각이 따라 바뀜(23:00+90분 → 00:30), 수업 길이 원장 전용·30~480 범위, 캠퍼스별 값. 7건 Red(404) 확인. 웹 8건 Red(모듈 없음) → Green: 블록 위치(`top`)·길이(`height`=120px)·시간 축, 겹친 수업 50% 폭, 미등록 묶음, 좁은 화면 요일별 목록, 강사 화면엔 선택·이름 없음, 원장 강사별 거르기(담당 없음 포함), 빈 상태, 수업 길이 저장 본문.
- AC-38 대응: ① 강사 담당 반만 — API 역할 스코프(기존 `_visible_classes` 재사용) ② 원장 전체+강사 이름+강사별 — API·웹 ③ 수업 길이 변경 → 블록 길이 — API `end_time`·웹 `height`가 `class_minutes`를 따름 ④ 미등록 — API·웹 ⑤ 비활성 제외 — API.
- 구현 결정(기준선 안): 강사 선택지는 `/users` 대신 시간표 응답의 강사 이름에서 만든다(DCR-008 — 강사가 `/users`를 부를 필요가 없게 서버가 이름을 붙임). 격자는 768px(`md`) 이상, 그 아래는 요일별 목록. 자정을 넘기는 수업은 격자에서 24시로 자른다.
- 스크린샷(1280·390px, 합성 7개 수업): 겹친 두 수업이 나란히 설 때 "18:00~20:0"처럼 시각이 잘려, `~` 뒤에서 줄이 바뀌게 고쳤다. `ops/verify/responsive.py`에 화면과 응답 스텁 추가.
- 검증: API 229건 통과(1 skip, 기존), 웹 102건, `npm run build`, VER-35 통과(주간 시간표 390·768·1280px 포함).

### 2026-09-24 — 수업 길이 기본값 4시간 (DCR-010, 기준선 v11)

- 사용자 지시: "수업시간은 클래스당 4시간으로 수정해줘. 2시간이 아니라 4시간이래"(클라이언트 확인). DCR-008의 "2시간(120분)"을 정정한다.
- 승인된 FR-42의 기본값이 바뀌므로 [DCR-010](./DCR-010-수업-길이-4시간.md)으로 기록하고 사용자 지시를 승인 근거로 기준선 `v11` 발행. 구조·계약·데이터 모델은 그대로다.
- TDD: `test_timetable.py`의 기본값 기대치 240·끝 시각 22:00으로 바꿔 2건 Red(`120 == 240`) → `DEFAULT_CLASS_MINUTES = 240`으로 Green. 웹의 응답 없을 때 대체값도 240. VER-35 스텁을 4시간 수업으로 바꿔 격자 스크린샷 확인(겹친 수업 배치 정상).
- 테스트 운영은 수업 길이를 저장한 적이 없어 배포만으로 240분이 된다.
- 검증: API 229건 통과(1 skip, 기존), 웹 페이지 테스트 77건, `npm run build`, VER-35 통과.

### 2026-09-24 — 반 이름 검토와 DCR-011 제안

- 사용자 질문: 반 이름이 "고1 인A"로 보이는 이유. 조사 결과 테스트 운영 DB의 반 5·6은 **이름 칸에 "고1 인A"·"고1 인B", 학년 칸에 "1"로 입력된 것**이다(시스템이 합치지 않는다). 반 관리 목록만 `이름 · 학년`, 나머지 화면은 이름만 보인다. 이름을 "인A"로 바꿔도 모든 기록은 반 ID로 연결되어 유지되고, 학부모 발송·리포트에는 반 이름이 없다. 다만 학년이 다른 같은 이름의 반이 생기면 이름만 보이는 화면에서 구분되지 않는다고 보고했다. 테스트 운영 데이터는 바꾸지 않았다.
- 사용자 요청: 원장이 반 이름을 바꿀 수 있는지, 강사도 가능해야 한다. 조사: 원장은 가능, 강사는 서버가 막는다(`require_director`). 권한 매트릭스는 모호("반 생성·삭제 불가"만). **강사 화면에 반 등록 폼·[수정]·[비활성]이 보이는데 누르면 오류만 난다**(결함).
- 사용자 결정: 강사 수정 범위 "이름·학년·시간표"(담당 반 한정, 담당 지정·활성·삭제는 원장만).
- [DCR-011](./DCR-011-강사의-담당-반-수정.md) 작성, 승인 요청.
- **사용자 승인(2026-09-24 23:45) → 기준선 `v12` 발행.** FR-07·역할별 권한 보강, AC-40, 권한 매트릭스, VER-40. TASK-67로 구현.

### 2026-09-24 — TASK-67 강사의 담당 반 수정 완료 (DCR-011, 기준선 v12)

- 선행 테스트: API [`test_class_teacher_edit.py`](../../../apps/api/tests/test_class_teacher_edit.py) 6건 — 강사가 담당 반 이름·학년·시간표 수정 200·반영, 담당 아닌 반 403·무변경, 담당 반의 `teacher_id`·`is_active` 변경 403·무변경(2건), 반 등록·삭제 403 유지, 감사 로그 `class.update`(detail `name`, target `class:{id}`). 2건 Red(403·로그 없음) — 나머지 4건은 기존 원장 전용 검사로 이미 통과하던 거부 규칙이며 새 구현 뒤에도 유지되는지 지키는 테스트다. 웹 3건 추가(강사 화면에 등록 폼·[비활성]·활성 여부·담당 강사 없음, 비활성 반에서도 삭제 없음, 이름 수정 본문에 현재 담당·활성 값) — 2건 Red.
- **기존 웹 테스트 조정:** `ClassesPage.test.tsx`의 `renderPage()` 기본값을 원장 화면(`isDirector`)으로 바꿨다. 기존 테스트는 반 등록·비활성·삭제 같은 원장 기능을 다루므로 의미는 같다. 강사 로그인 시 `/users`를 부르지 않는 기존 테스트만 강사 화면으로 명시했다.
- 구현 결정(기준선 안): 강사의 감사 로그 detail은 바뀐 칸 이름만 적는다(값은 적지 않음). 원장의 반 수정은 기존처럼 감사 로그를 남기지 않는다(DCR 범위 밖). 등록 폼의 담당 강사 칸은 이제 원장 블록 안에 있어 중복 조건을 없앴다.
- 검증: API 235건 통과(1 skip, 기존), 웹 105건, `npm run build`, VER-35 통과.

### 2026-09-25 — 로그인 12시간 만에 끊기는 버그와 끊김 안내 (경량 경로, 버그 수정)

- 사용자 보고: 강사 계정을 만들 때 "인증이 필요합니다". 테스트 운영 조사: 쓰던 세션 33은 02:35:08(UTC) 로그인, 마지막 정상 요청 14:35:05 — **로그인 후 정확히 12시간 직전**. 이후 `GET/POST /api/users`가 쿠키 없이 401. 서버 세션은 살아 있었다.
- 원인: 설계는 "유휴 만료 12시간"인데 쿠키 `Max-Age`를 **로그인 때만** 줘서 브라우저 쿠키가 로그인 12시간 뒤에 무조건 사라졌다(서버의 유휴 검사와 불일치). 함께 발견: 사용 중 401이 나도 화면이 그대로 남아 서버 메시지만 보였다.
- 재현 테스트 먼저: API `test_auth.py` — 로그인 뒤 `GET /auth/me`·`GET /students` 응답이 같은 토큰의 쿠키를 `Max-Age=43200`·HttpOnly·SameSite=Lax로 다시 주는지(Red: set-cookie 없음), 거부된 요청은 쿠키를 주지 않음. 웹 `App.test.tsx` — 로그인 확인 뒤 다른 요청이 401이면 로그인 화면 + "로그인이 만료되었습니다. 다시 로그인해 주세요."(Red), 처음부터 미로그인이면 안내 없음, 로그인 실패(401) 메시지는 그대로.
- 수정: `auth._set_session_cookie`로 쿠키 설정을 한곳에 모으고 `current_user`가 통과할 때마다 다시 설정(슬라이딩). 웹 `api.ts`가 `/auth/login`·`/auth/me` 외의 401에서 `mathdesk:session-expired` 이벤트를 알리고(`request`와 파일 업로드 2곳), `App`이 받아 로그인 화면으로 돌아가며 안내를 띄운다.
- 동작·계약 변경 없이 설계(유휴 12시간)대로 되돌리는 버그 수정이라 경량 경로. 파일을 직접 돌려주는 응답(이미지·다운로드)은 쿠키를 다시 주지 않지만 같은 화면의 다른 요청이 늘려 준다.
- 검증: API 237건 통과(1 skip, 기존), 웹 108건, `npm run build`, VER-35 통과, 만료 안내 화면 스크린샷 확인.

### 2026-09-25 — 테스트 운영 점검: 등원 0/0명, 반 배정 방법, 출결 잠금 (조사만, 코드 변경 없음)

- **"등원 현황 0/0명":** 활성 반 고1 인A·인B에 배정(enrollment)이 0건이었다. 학생 47명은 모두 비활성으로 돌린 옛 합성 반(고2 윤B·윤C·고1 윤D)에 배정. 코드 결함 아님 — 원인은 데이터. 사용자에게 [명단] 배정 방법, 배정 시작일이 항상 오늘이라 지난 수업일엔 안 보이는 점, 빈 반 안내가 없는 점을 보고.
- **반 배정·변경 방법 질문:** 학생 화면엔 반 선택이 없고 반 목록의 [명단]에서 배정·해제(원장 전용). 이동은 이전 반 [해제] + 새 반 [배정] 두 단계, 한 학생이 여러 반에 동시 배정 가능(해제를 빠뜨리면 두 반 모두에 나온다), 비활성 반 명단은 [비활성 포함]을 켜야 보인다.
- **"출결 버튼이 눌리지 않음":** 고1 인A 9/25 수업이 9/25 00:08(KST)에 `director`가 [확인]을 눌러 출결 확정(`attendance.confirm`) 상태였다 — FR-09 설계대로의 잠금. 잠금 표시는 버튼이 약간 흐려지는 것과 표 아래 [편집] 버튼 이름뿐이고, 비활성 버튼이라 눌러도 반응이 없었다(스크린샷으로 확인). 사용자도 "눈치채기 쉽지 않았다".
- **사용자 결정·요청(2026-09-25):**
  - 학생/반 관리를 [학생]·[반] 하위 탭으로 분리 → TASK-68(내 의견: 찬성, 주소별 탭 권장)
  - 빈 반 안내 → TASK-69, 반 배정 시작일 선택 → TASK-71(FR-08 안의 화면 미구현분이라 DCR 없음)
  - 출결 잠금 알림은 권고안(안내 띠 + 잠긴 칸 클릭 피드백 + 버튼 이름·열 🔒) 그대로, **확정한 사람 이름은 표시 안 함** → TASK-70
  - 권장 순서 3→1→4→2 = **TASK-68 → 69 → 70 → 71**
  - 결정 전 제안: 학생 목록에 "반" 칸, 명단 다중 배정·반 옮기기 한 동작(아래 인계 절)
- 테스트 운영 데이터(클라이언트 입력 가능성)는 바꾸지 않았다.

### 2026-09-25 — TASK-68 학생/반 관리 탭 분리 완료

- 선행 테스트: 웹 [`App.test.tsx`](../../../apps/web/src/App.test.tsx) 3건 — `/students`에서 학생 관리만 보이고 [학생] 탭·상단 "학생/반 관리"가 선택 상태, `/students/classes`에서 반 관리만 보이고 [반] 탭·상단 메뉴가 선택 상태, 탭을 눌러 오가기. 3건 Red(두 화면이 한 주소에 함께 있음·반 주소 없음·탭 없음) → Green. `mountApp`에 시작 주소 인자를 더했다.
- 구현: [`components/ManageTabs.tsx`](../../../apps/web/src/components/ManageTabs.tsx)가 탭 링크(`NavLink`) + `Outlet`을 그리는 레이아웃 라우트다. `App.tsx`에서 `/students` 아래 `index`(학생)·`classes`(반)로 나눴다. 두 화면 사이 간격이던 `StudentsPage`의 `mb-8`은 뺐다.
- **계획과 다른 점(내부 세부):** 계획은 반 탭 주소를 `/classes`로 적었으나 `/students/classes`로 정했다. 상단 메뉴 `NavLink('/students')`가 하위 주소에서 저절로 선택 상태가 되어 `AppShell` 변경이 필요 없다(최소 diff). 사용자에게 보이는 동작(탭별 주소, 새로 고침·뒤로 가기 유지, 메뉴 선택 표시)은 같다. TASK-69의 안내 링크는 이 주소를 쓴다.
- 설계 문서: `docs/design.md`에 라우트·화면 구성 표기가 없어 갱신할 곳이 없었다(기준선 의미 변경 없음).
- 검증: 웹 111건 통과(23 파일), `npm run build`, VER-35 통과 — BROWSE에 "학생 관리 `/students`"·"반 관리 `/students/classes`"를 나눠 넣고 390·768·1280px 모두 넘침 없음. 1280·390px 스크린샷으로 탭 모양 확인(390px 학교명 줄바꿈은 기존 기능 공백 항목 그대로).
- 배포: 백업 `~/mathdesk-backups/20260925-003503` 후 `docker compose -f compose.testops.yaml up -d --build`. 공개 주소 `/api/health` ok, `/students/classes` 200, 제공 번들 `index-BRr0eOPi.js`가 로컬 빌드와 같음.

### 2026-09-25 — TASK-69 빈 반 안내 완료

- 선행 테스트: 웹 [`DailyPage.test.tsx`](../../../apps/web/src/pages/DailyPage.test.tsx) 2건 — 그 수업일 명단이 0명이면 "이 날짜에 이 반에 배정된 학생이 없습니다" 안내와 `/students/classes` 링크, 학생이 있으면 안내 없음. 앞 1건 Red(안내 없음) → Green, 뒤 1건은 안내가 엉뚱하게 뜨지 않는지 지키는 테스트라 처음부터 통과. 링크 때문에 `renderPage()`를 `MemoryRouter`로 감쌌다.
- 구현: `DailyPage.tsx` 표 카드 안, 표 바로 아래에 안내 문단(`records.length === 0`일 때). 문구는 계획대로 "이 날짜에"를 넣어 배정은 있으나 시작일이 수업일 뒤인 경우도 틀리지 않게 했다. "0 / 0명" 표시는 그대로 둔다.
- 확인한 것: [명단] 버튼은 강사 화면에도 보여 링크 대상은 역할과 관계없이 같다(배정 자체는 원장 전용 — 기존 규칙).
- 검증: 웹 113건 통과, `npm run build`, VER-35 통과, 빈 반 일일 입력 1280px 스크린샷 확인.
- 배포: 백업 `~/mathdesk-backups/20260925-003813` 후 `up -d --build`, `/api/health` ok, 제공 번들 `index-DWf6TXZ9.js` = 로컬 빌드. 테스트 운영의 고1 인A·인B(배정 0건)에 이 안내가 보이게 된다.

### 2026-09-25 — TASK-70 출결 잠금 알림 강화 (배포, 사용자 육안 확인 대기)

- **범위: 대시보드에도 적용.** 계획의 변경 대상은 일일 입력뿐이었으나 대시보드의 "반별 출결 체크"도 같은 출결 칸·[확인]/[편집]·잠금을 쓴다(사용자가 겪은 화면이 어느 쪽인지 기록에 없음). 한쪽만 바꾸면 화면마다 버튼 이름이 달라지므로 공용 컴포넌트에 넣어 두 화면 모두 적용했다. 동작 규칙(FR-09)은 그대로다.
- 선행 테스트: 웹 [`DailyPage.test.tsx`](../../../apps/web/src/pages/DailyPage.test.tsx) 3건 — 잠김 시 띠("9/18 출결이 확정되어 잠겨 있습니다(M/D HH:MM 확정). 과제·테스트는 그대로 입력할 수 있습니다.")·열 제목 "출결 상태 🔒"·띠의 [확정 해제]가 `unlock` 호출, 잠긴 칸의 [출석] 클릭·사유 입력이 값을 바꾸지 않고 칸 옆에 "확정됨 · [확정 해제] 후 수정", 잠금 전 버튼 이름 [출결 확정]·열 제목에 🔒 없음. [`DashboardPage.test.tsx`](../../../apps/web/src/pages/DashboardPage.test.tsx)의 기존 확정 테스트를 새 이름·`aria-disabled`·띠·칸 옆 안내로 갱신. 4건 Red → Green.
- 구현: [`AttendanceButtons.tsx`](../../../apps/web/src/components/AttendanceButtons.tsx) — 잠긴 칸은 `disabled` 대신 `aria-disabled`(버튼)·`readOnly`(사유)로 두어 클릭을 받고, 클릭 시 칸 옆 안내 + `onLocked`. `AttendanceLockBanner`(경고색 띠, 잠긴 칸을 누를 때마다 1.2초 `animate-pulse`+테두리 강조, [확정 해제]). 확정한 사람 이름은 없다(서버 변경 없음). 확정 시각은 브라우저 로컬 시각 `M/D HH:MM`. 버튼 이름 [확인]/[편집] → [출결 확정]/[확정 해제], 요청 중에는 비활성. 대시보드 설명 문구도 "[출결 확정]"으로.
- **함께 고친 기존 결함(`ui/toggle.tsx`):** 눌린 토글(출석·B 등급 등)에 마우스를 올리면 outline의 `hover:bg-accent`가 남아 흰 글자만 남고 버튼이 사라져 보였다(잠금 전에도 재현, 스크린샷 확인). 잠긴 버튼이 이제 hover를 받아 더 자주 보이게 되므로 원인인 `TONE`에 같은 색의 `hover:`를 더했다. CSS hover라 jsdom 테스트 대신 스크린샷으로 확인.
- 검증: 웹 116건 통과, `npm run build`(tsc 포함), VER-35 통과. 스크린샷: 일일 입력 1280px(띠·🔒·칸 옆 안내·강조 테두리), 대시보드 390px(띠 줄바꿈 정상), 잠금 전 눌린 버튼 hover 전후.
- 배포: 백업 `~/mathdesk-backups/20260925-004537` 후 `up -d --build`, `/api/health` ok, 번들 `index-DL_DYCVq.js` = 로컬 빌드.
- 남은 것: 사용자 육안 확인(계획의 완료 조건). 테스트 운영의 고1 인A 9/25 수업이 확정 상태라 바로 볼 수 있다.

### 2026-09-25 — TASK-71 반 배정 시작일 선택 완료

- 선행 테스트(웹 [`ClassesPage.test.tsx`](../../../apps/web/src/pages/ClassesPage.test.tsx) 3건): 시작일 칸(기본 오늘)에서 고른 날짜로 `POST` 본문 `start_date`, 명단에 시작 전인 배정은 남고 끝난 배정은 빠짐(목이 서버의 `on` 필터를 그대로 흉내), 시작 전인 배정은 [해제] 대신 [취소] → `DELETE`. 모두 Red → Green. 기존 "오늘 배정" 테스트는 기본값 확인으로 그대로 통과.
- 구현: `RosterDialog`에 "시작일" 날짜 칸. 명단 조회를 `?on=오늘` 대신 전체 조회 + 화면에서 `end_date` 없음 또는 `>= 오늘`만 남기기로 바꿨다 — 오늘 기준 조회는 미래 시작 배정을 숨겨 학생 선택지에 다시 나오고 중복 배정을 부른다(계획의 위험 항목). 시작 전인 배정은 종료일(오늘)이 시작일보다 앞서 해제가 422로 거절되므로 서버의 "오등록 취소" `DELETE`를 쓰는 [취소]를 보인다(`api.ts` `cancelEnrollment`) — 시작 전이라 지울 이력이 없다.
- **서버 입력 검증 추가(`masterdata.create_enrollment`):** 같은 반·같은 학생에 기간이 겹치는 배정을 422 "이 반에 기간이 겹치는 배정이 이미 있습니다."로 거절. 겹치면 일일 명단 조인에서 학생이 두 번 나온다. 시작일을 고를 수 있게 되며 화면에서도 만들 수 있게 되어 신뢰 경계에서 막았다. 재현 테스트 [`test_masterdata.py`](../../../apps/api/tests/test_masterdata.py) `test_enrollment_cannot_overlap_an_existing_period` Red(`201 == 422`) → Green. 테스트 운영 DB의 기존 겹침 배정은 0건(읽기 전용 조회).
- 해제 종료일 선택은 **검토 후 보류**: 해제의 주 용도는 반 이동이며, 사용자 결정 대기 중인 제안 ②(반 옮기기 한 동작)가 날짜를 함께 다룬다. 그 결정 때 같이 정한다. 기능 공백 항목은 그대로 둔다.
- 완료 조건 "과거 날짜 배정이 해당 수업일 일일 입력에 보임": 서버는 이미 `start_date <= 수업일`로 명단을 만들며 기존 `test_daily_lists_students_enrolled_on_that_date`(03-02 시작 배정이 09-18 명단에)가 보장한다. 화면은 그 `start_date`를 보낼 수 있게 된 것이다.
- 검증: API 238건 통과(1 skip, 기존 — 겹침 검사 전 전체 실행, 뒤이어 생성 시 종료일 검사를 빼고 관련 33건 재실행), 웹 119건, `npm run build`, VER-35 통과, 명단 대화상자 1280·390px 스크린샷(한 줄 배치 정상).
- 배포: 백업 `~/mathdesk-backups/20260925-005432` 후 `up -d --build`(마이그레이션 없음), `/api/health` ok, 번들 `index-Di57t3s1.js` = 로컬 빌드.

### 2026-09-25 — 사용자 결정: 제안 ①·② 진행

- 사용자: "2번 진행하자" → 확인 질문에 "제안 ①·② 둘 다". ① 학생 목록 "반" 칸 = TASK-72, ② 명단 다중 배정·반 옮기기 = TASK-73(해제 종료일은 옮기기가 시작일 전날로 정함).
- DCR 판단: 둘 다 기존 API(`GET /classes?include_inactive=true`, `GET/POST/PATCH /classes/{id}/enrollments`) 조합으로 되며 요구사항(FR-05 표시, FR-08 배정·해제)·데이터·권한 변경이 없다 → DCR 없이 경량 경로. 서버 집계 필드나 일괄 배정 엔드포인트는 계약 추가라 택하지 않았다.

### 2026-09-25 — TASK-72 학생 목록 "반" 칸 완료

- 선행 테스트: 웹 [`StudentsPage.test.tsx`](../../../apps/web/src/pages/StudentsPage.test.tsx) 1건 — "반" 열 제목, 두 반 배정 학생 "고1 인A, 고1 인B", 비활성 반 "고2 윤B (비활성)", 미배정 "—". Red(열 없음) → Green.
- 구현: [`lib/currentClasses.ts`](../../../apps/web/src/lib/currentClasses.ts) `useCurrentClasses()` — 반 목록(비활성 포함, 쿼리 키 `['classes', true]`는 반 탭과 공유) + 반마다 `?on=오늘` 명단(`['enrollments', 반 id, 오늘]` — 명단 대화상자의 `['enrollments', 반 id]` 무효화에 함께 걸린다). TASK-73이 같은 훅을 쓴다. 강사는 서버가 주는 담당 반 범위만 보인다.
- 함께 해결: 390px 학교명 글자 단위 줄바꿈(기능 공백 항목) — 학교·반 칸에 `break-keep`(한국어 단어 단위 줄바꿈).
- 검증: 웹 120건, `npm run build`, VER-35 통과, 1280·390px 스크린샷.
- 배포: 백업 `~/mathdesk-backups/20260925-123143` 후 `up -d --build`, `/api/health` ok, 번들 `index-Bg_fpzcD.js` = 로컬 빌드.

### 2026-09-25 — TASK-73 명단 다중 배정·반 옮기기 (배포, 사용자 확인 대기)

- 선행 테스트(웹 [`ClassesPage.test.tsx`](../../../apps/web/src/pages/ClassesPage.test.tsx)): 신규 3건 — [전체 선택]으로 후보 3명 → 학생별 `POST` 3건·"3명 배정했습니다", 가져올 반(비활성 반 9) 선택 시 그 반 학생만 후보, 옮기기 켜고 시작일 2026-09-01 → 학생마다 `POST` 뒤 이전 반 `PATCH end_date=2026-08-31`(순서까지 확인), 한 학생의 `POST`가 422면 그 학생의 이전 반은 건드리지 않고 "김정현: …" 실패 목록·"1명 옮겼습니다". 기존 단일 선택 테스트 3건은 체크 목록 조작으로 갱신(검증 내용은 같음). 6건 Red → Green.
- 구현(`RosterDialog`): 단일 학생 선택을 체크 목록(현재 반 함께 표시 — TASK-72의 `useCurrentClasses` 재사용)·[전체 선택]·"가져올 반"(전체 학생 / 다른 반, 비활성 포함)으로 바꿨다. 가져올 반을 고르면 "○○ 배정은 {시작일 전날}까지로 끝냅니다(반 옮기기)" 체크 칸. 실행은 학생마다 순차로 새 배정 → 성공 시 이전 반 종료(요청 사이 실패해도 학생이 어느 반에서도 빠지지 않고 두 반에 남아 보인다). 진행 중 버튼은 "처리 중…", 끝나면 완료 수와 실패 학생·사유. `lib/date.ts`에 `dayBefore`(로컬 달력 기준 전날)를 더하고 `today`와 날짜 형식을 공유하게 정리.
- TASK-71에서 보류한 해제 종료일은 옮기기가 "시작일 전날"로 정한다. 단독 [해제]의 종료일은 여전히 오늘(기능 공백 항목 유지).
- 검증: 웹 123건, `npm run build`(tsc 포함), VER-35 통과, 비활성 반 47명 옮기기 화면 1280·390px 스크린샷.
- 배포: 백업 `~/mathdesk-backups/20260925-123842` 후 `up -d --build`, `/api/health` ok, 번들 `index-C5tFVFAd.js` = 로컬 빌드.
- 남은 것: 사용자 확인(계획의 완료 조건). 테스트 운영에서 고1 인A [명단] → 가져올 반 "고2 윤B (비활성)" 등으로 실제 이동해 볼 수 있다(실데이터가 바뀌므로 사용자가 직접).

### 2026-09-25 — TASK-70 사용자 확인 완료

- 사용자: "Task70은 확인완료야. 73은 확인해볼게" → TASK-70 `completed`(2026-09-25 18:55). TASK-73은 사용자 확인 중.

### 2026-09-25 — 대시보드 항목 링크 검토 (조사만, 코드 변경 없음)

- 사용자 요청: 종합 대시보드 항목을 누르면 해당 화면으로 이동하는 링크 — 검토 후 구현 결정.
- 목적지안: 총 재원생 → 학생 탭 `/students`, 활성 N개 반 → 반 탭 `/students/classes`, 등원 현황·반별 출결 체크 → 일일 입력(같은 반·날짜), 금주 과제 완수율·주간테스트 평균 → 성적 통계(같은 반, 그 주 월~일 — 두 KPI는 선택 반·선택 날짜의 주 기준이고 통계 화면에 학생별 테스트 평균·과제 완수율이 있다), 지난 수업 진도 & 과제 → 일일 입력(그 수업일).
- 발견: ① 일일 입력·성적 통계·대시보드 모두 반·날짜를 화면 내부 상태로만 가져 주소로 받을 수 없다(`useSearchParams` 사용처 없음) — 링크만 달면 첫 번째 반·오늘로 열려 보던 반과 달라지고, 뒤로 가기 때 대시보드 선택도 초기화된다. 세 화면이 `?class=&date=`(통계는 `?class=&start=&end=`)를 초기값으로 읽어야 한다. ② 대시보드 출결 체크의 미저장 입력은 이동 시 사라진다(일일 입력에만 확인 창이 있다). ③ 대시보드는 휴대폰 지원 화면이지만 일일 입력은 데스크톱 전제. ④ 강사는 담당 반만 보이므로 권한 문제 없음. ⑤ FR-15는 KPI 표시만 정해 링크는 화면 탐색 추가 — DCR 불필요, 서버 변경 없음.
- 사용자 결정 대기: 구현 여부, 과제 완수율 목적지(통계 권장), 대시보드 자체 주소 유지(권장), 클릭 방식(타일 전체 권장).

### 2026-09-25 — TASK-74 대시보드 항목 링크 완료

- 사용자: 검토 결과를 보고 "진행시켜" → 권장안(타일 전체 클릭, 과제 완수율 → 성적 통계, 대시보드 주소에 반·날짜 유지)으로 진행.
- 선행 테스트 5건(모두 Red → Green): [`DashboardPage.test.tsx`](../../../apps/web/src/pages/DashboardPage.test.tsx) — `/?class=1&date=2026-09-20`으로 열면 그 값으로 조회하고, 47명 → `/students`, 활성 4개 반 → `/students/classes`, 등원 현황·"일일 입력에서 열기" → `/daily?class=1&date=2026-09-20`, 과제 완수율·테스트 평균 → `/stats?class=1&start=2026-09-14&end=2026-09-20`(그 주 월~일), "2026-09-18 수업 열기" → 그 수업일 일일 입력. 출결 미저장 입력이 있으면 확인 창(취소 시 머묾, 확인 시 이동), 없으면 묻지 않음. [`DailyPage.test.tsx`](../../../apps/web/src/pages/DailyPage.test.tsx)·[`StatsPage.test.tsx`](../../../apps/web/src/pages/StatsPage.test.tsx) — 주소의 반·날짜(통계는 시작·종료)로 조회. 대시보드·통계 테스트를 `MemoryRouter`로 감쌌다.
- 구현: `Stat`에 `to`·`onClick` — 타일 전체가 링크(마우스를 올리면 테두리 강조, 제목 줄 "→", 키보드 포커스 테두리). 재원생 타일은 두 목적지라 "47명"·"활성 N개 반"을 각각 텍스트 링크로. 대시보드의 반·날짜는 `useSearchParams`(선택 변경은 `replace`라 방문 기록이 쌓이지 않는다). 일일 입력·통계는 주소 값을 초기값으로만 읽는다(화면 안에서 바꾼 값은 주소에 남기지 않음). `lib/date.ts`에 `weekOf`(서버 `_week_bounds`와 같은 월~일). 출결 카드 설명에 `break-keep`(390px에서 링크 옆으로 좁아져 글자 단위로 끊겼다).
- 범위 밖으로 남긴 것: 상단 메뉴로 이동할 때의 미저장 확인은 기존에도 없다(`BrowserRouter`라 `useBlocker`를 쓸 수 없다). 대시보드에서 반·날짜를 바꿀 때의 미저장 확인도 기존대로 없다.
- 검증: 웹 128건, `npm run build`(tsc 포함), VER-35 통과. 실제 브라우저 실측: `/?class=2&date=2026-09-23`에서 과제 완수율 클릭 → `/stats?class=2&start=2026-09-21&end=2026-09-27`, 반·시작·종료 칸이 그 값, 뒤로 가기 → 대시보드가 반 2·9/23 유지. 1280·390px 스크린샷.
- 배포: 백업 `~/mathdesk-backups/20260925-192248` 후 `up -d --build`, `/api/health` ok, 번들 `index-B_VZdlhq.js` = 로컬 빌드.
...
| 배정 기간 겹침 거절 | REST 계약의 `POST /classes/{id}/enrollments`에 없던 422(같은 반·학생의 기간 겹침)를 더했다. 겹치면 일일 명단에 학생이 두 번 나오며, TASK-71로 화면에서 시작일을 고를 수 있게 되어 막았다. 기존 PATCH의 "종료일 < 시작일" 422와 같은 입력 검증이다 | 경미한 변경으로 처리, 사용자에게 보고 |
...
- 기능 공백(요구사항 충족 범위 밖의 후속 후보)
  - 단독 [해제]의 종료일은 화면에서 항상 오늘(과거 날짜는 API만). 반 옮기기(TASK-73)는 시작일 전날로 끝낸다
  ...
  - ~~390px 학생 표의 학교명 글자 단위 줄바꿈~~ → TASK-72에서 `break-keep`으로 해결
  - 이미지 OMR 업로드의 해상도 상한 없음(원장 전용·20MB)
  - 시행일 없는 시험의 재원 수는 서버 UTC 날짜 기준

## 재개 지점

- 다음 작업: 사용자 확인 대기 — [TASK-73](../../plan.md#task-73-명단-다중-배정반-옮기기-fr-08)(다중 배정·반 옮기기, 사용자가 테스트 운영에서 확인 중). 2026-09-25 세션 완료: TASK-68~72·74. 이후는 사용자가 줄 새 항목

## 인계

- 다음 단계 또는 워크플로우: 사이클 3 — 테스트 운영 피드백 반영(wf-implement 경량 경로: TASK-68~72·74 완료, TASK-73 사용자 확인 대기). 요구사항·권한·데이터가 바뀌면 wf-design(DCR). 배포처 결정은 wf-design
- 시작 조건: 새 항목은 사용자 지시 후. TASK-43 재개는 Anthropic API 키가 `.env`에 있을 때
- 입력 문서와 기준선: [PLAN-mathdesk](../../plan.md), [REQ-mathdesk](../../requirements.md) `v12`, [DESIGN-mathdesk](../../design.md) `v12`, [결정 등록부](../../decisions.md)(ADR-001~012, DCR-001~011 모두 `approved`)
- 완료된 항목: 기준선 v1~v12, ADR-001~012, DCR-001~011, 사이클 1·2 전체(TASK-43 보류 제외), 사이클 3의 TASK-61~72·74(안내문, 학생·반 삭제, 반 시간표, 계정·비밀번호, 담당 강사, 주간 시간표, 강사의 담당 반 수정, 학생/반 탭 분리, 빈 반 안내, 출결 잠금 알림, 배정 시작일, 학생 목록 반 칸, 대시보드 항목 링크), 수업 길이 기본 240분(DCR-010), 로그인 유지 버그 수정(2026-09-25)
- 미완료 항목: TASK-73 사용자 확인(진행 중), TASK-43(보류), 사이클 3 이관 준비 TASK-54~60(클라이언트 승인 대기)
...
- 테스트 운영: **작업이 끝날 때마다 바로 배포한다**(사용자 지시 2026-09-24). 최신 앱 변경(TASK-74 대시보드 항목 링크 2026-09-25)까지 배포됨
- 차단 요인: TASK-43은 API 키 대기(사용자 보류). 외부 조건 미검증 항목은 [미완료 항목](#미완료-항목)
- 다음 행동: **사용자의 TASK-73 확인 결과를 받는다**(재개 시 먼저 결과를 묻는다). 확인되면 plan.md의 TASK-73을 `completed`(완료 시점 기입)·트리 `[✓]`·mermaid `done`으로, 문제가 나오면 재현 테스트부터 고친다. 사용자가 새 항목을 주면 plan.md에 TASK-75부터 등록하고 TDD로 진행. 각 TASK 끝나면 커밋·push·테스트 운영 배포(`ops/backup.sh` → `docker compose -f compose.testops.yaml up -d --build` → 공개 주소 번들 일치 확인)
- TASK-73 확인 방법(사용자에게 안내함): 반 탭 → 고1 인A [명단] → 가져올 반 "고2 윤B (비활성)" 등 → [전체 선택] → "반 옮기기" 체크 → [N명 옮기기]. 지난 수업일부터 보이게 하려면 시작일을 그 날짜로. 실데이터가 바뀌므로 Claude는 실행하지 않았다
- 사용자 결정(2026-09-25): 제안 ①·② 모두 진행 → TASK-72(완료)·TASK-73(확인 대기). 대시보드 항목 링크는 권장안으로 진행 → TASK-74(완료). TASK-70 사용자 육안 확인 완료
