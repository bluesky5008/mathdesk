...
- 다음 작업: [TASK-68 학생/반 관리 탭 분리](../../plan.md#task-68-학생반-관리-탭-분리) → [TASK-69 빈 반 안내](../../plan.md#task-69-빈-반-안내-일일-입력) → [TASK-70 출결 잠금 알림 강화](../../plan.md#task-70-출결-잠금-알림-강화) → [TASK-71 반 배정 시작일 선택](../../plan.md#task-71-반-배정-시작일-선택-fr-08)
- 사용자가 정한 순서(2026-09-25): 권장 순서 3→1→4→2(= 위 순서)

## 인계

- 다음 단계 또는 워크플로우: 사이클 3 — 테스트 운영 피드백 반영(wf-implement 경량 경로: TASK-68~71). 요구사항·권한·데이터가 바뀌면 wf-design(DCR). 배포처 결정은 wf-design
- 시작 조건: TASK-68~71은 바로 착수 가능(모두 승인된 요구사항 안의 화면 작업, DCR 불필요 확인). TASK-43 재개는 Anthropic API 키가 `.env`에 있을 때
...
- 완료된 항목: 기준선 v1~v12, ADR-001~012, DCR-001~011, 사이클 1·2 전체(TASK-43 보류 제외), 사이클 3의 TASK-61~67(안내문, 학생·반 삭제, 반 시간표, 계정·비밀번호, 담당 강사, 주간 시간표, 강사의 담당 반 수정), 수업 길이 기본 240분(DCR-010), 로그인 유지 버그 수정(2026-09-25)
- 미완료 항목: **TASK-68~71(다음 착수, 아래 순서)**, TASK-43(보류), 사이클 3 이관 준비 TASK-54~60(클라이언트 승인 대기)
...
- 테스트 운영: **작업이 끝날 때마다 바로 배포한다**(사용자 지시 2026-09-24). 최신 앱 변경(로그인 유지 버그 수정 2026-09-25)까지 배포됨
- 차단 요인: TASK-43은 API 키 대기(사용자 보류). 외부 조건 미검증 항목은 [미완료 항목](#미완료-항목)
- 다음 행동: **[TASK-68 학생/반 관리 탭 분리](../../plan.md#task-68-학생반-관리-탭-분리) 착수**(TDD) → [TASK-69 빈 반 안내](../../plan.md#task-69-빈-반-안내-일일-입력) → [TASK-70 출결 잠금 알림 강화](../../plan.md#task-70-출결-잠금-알림-강화) → [TASK-71 반 배정 시작일 선택](../../plan.md#task-71-반-배정-시작일-선택-fr-08). 각 TASK 끝나면 커밋·push·테스트 운영 배포
