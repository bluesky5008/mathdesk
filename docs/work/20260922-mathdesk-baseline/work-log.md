...
- 기능 공백(요구사항 충족 범위 밖의 후속 후보)
  - 수강 해제 종료일은 화면에서 항상 오늘(과거 날짜는 API만) → TASK-71에서 검토 후 보류, 제안 ②(반 옮기기 한 동작) 결정 때 함께 정한다
...
- 다음 작업: [TASK-73 명단 다중 배정·반 옮기기](../../plan.md#task-73-명단-다중-배정반-옮기기-fr-08). 완료: TASK-68·69·71·72, TASK-70은 사용자 육안 확인 대기
- 사용자가 정한 순서(2026-09-25): 권장 순서 3→1→4→2(= 위 순서)

## 인계

- 다음 단계 또는 워크플로우: 사이클 3 — 테스트 운영 피드백 반영(wf-implement 경량 경로: TASK-73 진행, TASK-68·69·71·72 완료, TASK-70 육안 확인 대기). 요구사항·권한·데이터가 바뀌면 wf-design(DCR). 배포처 결정은 wf-design
- 시작 조건: TASK-73 바로 착수 가능(DCR 불필요 판단, 2026-09-25 기록). TASK-43 재개는 Anthropic API 키가 `.env`에 있을 때
...
- 미완료 항목: **TASK-73(진행 중)**, TASK-70 사용자 육안 확인, TASK-43(보류), 사이클 3 이관 준비 TASK-54~60(클라이언트 승인 대기)
...
- 테스트 운영: **작업이 끝날 때마다 바로 배포한다**(사용자 지시 2026-09-24). 최신 앱 변경(TASK-72 학생 목록 반 칸 2026-09-25)까지 배포됨
- 차단 요인: TASK-43은 API 키 대기(사용자 보류). 외부 조건 미검증 항목은 [미완료 항목](#미완료-항목)
- 다음 행동: **[TASK-73](../../plan.md#task-73-명단-다중-배정반-옮기기-fr-08) 구현**(TDD, `useCurrentClasses` 재사용). TASK-70은 사용자가 육안 확인하면 `completed`로. 각 TASK 끝나면 커밋·push·테스트 운영 배포
