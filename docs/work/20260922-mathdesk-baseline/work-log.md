...
- 진행 중인 작업: [TASK-49 마스터 데이터 수정 기능 보완](../../plan.md#task-49-마스터-데이터-수정-기능-보완) — TASK-50 완료, TASK-51·52 남음
- 마지막 완료 작업: [TASK-50 학생 수정·퇴원 화면](../../plan.md#task-50-학생-수정퇴원-화면) (2026-09-24 01:05)
...
- TASK-51·TASK-52(마스터 데이터 수정 보완의 나머지), TASK-24~TASK-39·TASK-43(사이클 2)
...
- 학생 관리 화면의 육안 확인 미수행 — 수정 대화상자와 `퇴원 포함` 토글은 vitest·빌드·반응형 측정만 거쳤다
...
- 다음 작업: [TASK-51 반 수정·활성 여부](../../plan.md#task-51-반-수정활성-여부-api-보완-포함) — **API부터**. 그 뒤 [TASK-52](../../plan.md#task-52-수강-배정해제-화면)(수강 배정·해제), 이어서 사이클 2 기능 재개([TASK-24](../../plan.md#task-24-m5-성적-통계))
- 사용자가 지정한 순서(2026-09-24): 모바일(완료) → 마스터 데이터 수정(TASK-50 완료) → 사이클 2 재개
- 먼저 확인할 사항: [계획 트리](../../plan.md#계획-트리)의 현재 상태, `git status`가 깨끗한지, `docker compose ps`로 개발 스택 기동 여부
- 필요한 문서: [FR-07](../../requirements.md#기능-요구사항), [FR-08](../../requirements.md#기능-요구사항), [DES-04](../../design.md#컴포넌트와-책임), [TASK-51·52 정의](../../plan.md#task-51-반-수정활성-여부-api-보완-포함)
...
- **이 작업의 핵심 제약**
  ...
  - TASK-50은 완료했다. 그 화면이 쓴 방식(행별 `수정` 대화상자 + 전체 필드 PATCH + 목록 토글)을 TASK-51에도 그대로 쓰면 된다. **`PATCH`는 전치환이라 폼이 다루지 않는 필드는 지워진다** — 반에도 같은 함정이 있다(`schedules`).
...
- 다음 단계 또는 워크플로우: wf-implement 구현 — TASK-51부터
- 시작 조건: 충족됨 — 기준선 `v6` 승인(2026-09-23), TASK-50 완료, `git status` 깨끗
...
- 완료된 항목: 기준선 v1~v6 승인, ADR-001~012, DCR-001~005, 사이클 1 전체(TASK-01~TASK-22), TASK-23, TASK-40~TASK-42, 시각 설계 TASK-44~TASK-48, 모바일 TASK-53, TASK-50 — 작업 53건 중 32건
- 미완료 항목: TASK-51·TASK-52(마스터 데이터 수정 보완), TASK-24~TASK-39·TASK-43(사이클 2)
- 차단 요인: 없음
- 다음 행동: **TASK-51 반 수정·활성 여부, API부터.** `ClassIn`에 `is_active` 필드가 없고 `update_class`가 이를 다루지 않는다. 선행 테스트(Red)는 "`is_active`를 `false`로 PATCH하면 `GET /classes` 기본 목록과 대시보드의 활성 반 수에서 빠진다" — 대시보드 쪽은 [`stats.py`](../../../apps/api/src/mathdesk/stats.py)가 이미 `Klass.is_active`를 세므로 반 목록의 기본 제외 여부를 먼저 정해야 한다(학생은 화면에서 걸렀다. 반은 일일 입력·대시보드의 반 선택 목록에도 쓰이므로 서버에서 거르는 편이 맞는지 확인할 것). 그다음 `ClassesPage`에 수정 대화상자를 넣는다
