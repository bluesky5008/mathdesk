...
- 다음 행동: **[TASK-61 클라이언트 안내문 작성](../../plan.md#task-61-클라이언트-안내문-작성)**(사용자 지시로 다음 작업). 재료는 작업 기록의 "사이클 3 계획 수립"·월 비용 산출(반 8개)과 `ops/messaging-setup.md`. 그 뒤 사이클 3 흐름: 클라이언트 요청이 오면 [TASK-54](../../plan.md#task-54-클라이언트-피드백-반영)로 반영(테스트 운영 즉시 배포), 병행해서 [TASK-55](../../plan.md#task-55-외부-계정도메인-준비), 사용자가 원하면 [TASK-56 DCR-006](../../plan.md#task-56-dcr-006-운영-배포처-결정). 클라이언트 인수 승인(TASK-58) 전에는 AWS에 아무것도 만들지 않는다
