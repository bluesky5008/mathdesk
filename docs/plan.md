...
- 다음 행동: **사이클 3(운영 이관) 계획 수립(2026-09-24).** 다음 작업은 [TASK-61 클라이언트 안내문](#task-61-클라이언트-안내문-작성)이다. 클라이언트 피드백은 현재 테스트 운영에서 반영하고([TASK-54](#task-54-클라이언트-피드백-반영)), 외부 계정 준비([TASK-55](#task-55-외부-계정도메인-준비))를 병행하며, 클라이언트 인수 승인([TASK-58](#task-58--클라이언트-인수-승인)) 뒤 AWS로 이관한다([TASK-59](#task-59-운영-이관-실행과-검증)). 배포처 변경은 [TASK-56 DCR](#task-56-dcr-006-운영-배포처-결정)이 먼저다.
...
│
└─ [ ] 사이클 3 — 운영 이관 (계획 2026-09-24) ......... pending (0/8)
   ├─ [ ] TASK-61 클라이언트 안내문 작성 ............. 다음 작업(사용자 지시)
   ├─ [ ] TASK-54 클라이언트 피드백 반영 (반복) ...... 현재 테스트 운영
   ├─ [ ] TASK-55 외부 계정·도메인 준비 (병행) ....... 클라이언트·사용자
   ├─ [ ] TASK-56 DCR-006 운영 배포처 결정 ........... wf-design, 사용자 승인
...
- 상태: pending — 사용자 지시로 다음 작업으로 등록(2026-09-24). 먼저 학생·반 삭제 문의를 처리한다
- 상위: 없음
...
- 완료 조건: DCR-006 `approved`, 새 기준선 발행
...
- 관련 요구사항과 설계: DCR-006, [TASK-43](#task-43-claude-실호출-검증)(Claude 실호출 — 운영에서 함께 검증)
