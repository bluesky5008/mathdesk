...
- 현재 결론 또는 상태: 작업 39건 중 TASK-02·TASK-03을 완료했다. TASK-01은 자식 2건이 남아 진행 중이다.
- 다음 행동: [TASK-04 인증과 세션](#task-04-인증과-세션)을 시작한다.
...
```text
mathdesk 구현 (기준선 v1, 작업 20260922-mathdesk-baseline)     in-progress (2/39)
│
├─ 사이클 1 — MVP (M0~M4) ........................... in-progress (2/22)
│  ├─ [▶] TASK-01 M0 기반 (분해 4, 2/4)
...
│  │   ├─ [ ] TASK-07 마스터 데이터 API
│  │   └─ [ ] TASK-08 마스터 데이터 화면 ............. depends: TASK-07
...
│  └─ [ ] TASK-22 ★ MVP 릴리스 관문 (사용자 승인) .... depends: TASK-21
│
└─ 사이클 2 — 확장 (M5~M9) .......................... pending (0/17)
   ...
   └─ [ ] TASK-39 ★ 최종 릴리스 관문 (사용자 승인) ... depends: TASK-38
```
...
flowchart TD
    ...
    T01 --> T04["TASK-04 인증·세션"]:::active
    T01 --> T05["TASK-05 권한·캠퍼스 스코프"]:::todo
    C1 --> T06["TASK-06 M1 학생/반 관리"]:::todo
    T06 --> T07["TASK-07 마스터 데이터 API"]:::todo
    T06 --> T08["TASK-08 마스터 데이터 화면"]:::todo
    C1 --> T09["TASK-09 M2 일일 수업 입력"]:::todo
    T09 --> T10["TASK-10 일일 기록 API (3 저장 단위)"]:::todo
    ...
    C1 --> T22[["★ TASK-22 MVP 릴리스 관문"]]:::gate
    T02 -. depends .-> T03
    T03 -. depends .-> T04
    T04 -. depends .-> T05
    T05 -. depends .-> T07
...
- 완료 조건: TASK-02~TASK-05가 모두 `completed`이고 [AC-01~AC-03](./requirements.md#인수-조건)이 통과한다
...
- 완료 조건: AC-02·AC-03 통과, 엔드포인트 순회 테스트가 누락 0건을 보고
...
- 완료 조건: [AC-05](./requirements.md#인수-조건) 시나리오(반 4개·학생 47명 등록)를 화면에서 수행하고 목록에 반영됨을 확인
...
- 상태: pending
- 상위: 없음
- 목표: MVP 결과를 원격 저장소에 반영한다. 외부·비가역 작업이므로 사용자 승인이 필수다.
- 관련 요구사항과 설계: [wf-implement 자율 진행과 승인](./plan.md#인계) 규칙 — 설계 승인은 push 권한이 아니다
- 변경 대상: 없음(저장소 상태 반영)
- 의존성: TASK-21
- 위험: 승인 없이 수행하면 비가역 변경이 된다
- 검증 방법: 승인 응답 기록 확인
- 완료 조건: 사용자 승인 후 push 완료, 승인 사실과 시점을 작업 기록에 남김
...
- 상태: pending
- 상위: 없음
- 목표: 전체 결과를 원격 저장소에 반영한다. 외부·비가역 작업이므로 사용자 승인이 필수다.
- 관련 요구사항과 설계: 없음(실행 권한 관문)
- 변경 대상: 없음(저장소 상태 반영)
- 의존성: TASK-38
- 위험: 승인 없이 수행하면 비가역 변경이 된다
- 검증 방법: 승인 응답 기록 확인
