...
└─ [ ] 사이클 3 — 운영 이관 (계획 2026-09-24) ......... in-progress (3/13)
   ├─ [▶] TASK-64 계정 관리·비밀번호 (FR-02, DCR-009) .. 선행
...
flowchart TD
    C3["사이클 3 — 운영 이관"]:::todo
    C3 --> T64["TASK-64 계정 관리·비밀번호 (FR-02, DCR-009)"]:::active
...
- 완료 조건: 원장이 화면에서 강사 계정을 만들고 그 계정으로 로그인할 수 있다
...
| VER-39 | AC-39 | TASK-64 | 본인 변경·재설정·변경 강제(서버 403)·세션 무효화·잠금 해제·감사 로그 API 테스트 + 강제 화면·변경·재설정 웹 테스트 |
