...
│
└─ [ ] 사이클 3 — 운영 이관 (계획 2026-09-24) ......... in-progress (13/21)
   ├─ [ ] TASK-73 명단 다중 배정·반 옮기기 (FR-08) ... 배포됨, 사용자 확인 대기
   ├─ [✓] TASK-74 대시보드 항목 링크 .................. 2026-09-25 19:23
   ├─ [✓] TASK-70 출결 잠금 알림 강화 ................ 2026-09-25 18:55
   ├─ [✓] TASK-72 학생 목록 "반" 칸 ................. 2026-09-25 12:34
   ├─ [✓] TASK-71 반 배정 시작일 선택 (FR-08) ........ 2026-09-25 00:56
   ├─ [✓] TASK-69 빈 반 안내 (일일 입력) ............. 2026-09-25 00:38
   ├─ [✓] TASK-68 학생/반 관리 탭 분리 ............... 2026-09-25 00:35
   ├─ [✓] TASK-67 강사의 담당 반 수정 (DCR-011) ........ 2026-09-24 23:51
...
flowchart TD
    C3["사이클 3 — 운영 이관"]:::todo
    C3 --> T68["TASK-68 학생/반 관리 탭 분리"]:::done
    C3 --> T69["TASK-69 빈 반 안내"]:::done
    C3 --> T70["TASK-70 출결 잠금 알림 강화"]:::done
    C3 --> T71["TASK-71 반 배정 시작일 선택 (FR-08)"]:::done
    T68 -. depends .-> T69
    C3 --> T72["TASK-72 학생 목록 반 칸"]:::done
    C3 --> T73["TASK-73 명단 다중 배정·반 옮기기 (FR-08)"]:::active
    T72 -. depends .-> T73
    C3 --> T74["TASK-74 대시보드 항목 링크"]:::done
    C3 --> T67["TASK-67 강사의 담당 반 수정 (DCR-011)"]:::done
    C3 --> T64["TASK-64 계정 관리·비밀번호 (FR-02, DCR-009)"]:::done
    C3 --> T65["TASK-65 반 담당 강사 지정 (FR-07)"]:::done
    C3 --> T66["TASK-66 주간 시간표 (DCR-008)"]:::done
    T64 -. depends .-> T66
    T65 -. depends .-> T66
    C3 --> T63["TASK-63 반 시간표 입력 (DCR-007)"]:::done
...
- 상태: completed (2026-09-25 19:23)
- 상위: 없음
- 목표: 종합 대시보드 항목을 누르면 해당 화면으로 이동한다. 재원생 수 → 학생 탭, 활성 반 수 → 반 탭, 등원 현황·반별 출결 체크 → 일일 입력(같은 반·날짜), 과제 완수율·테스트 평균 → 성적 통계(같은 반, 그 주 월~일), 지난 수업 → 일일 입력(그 수업일). 이동한 화면은 주소의 반·날짜로 열리고, 대시보드도 반·날짜를 주소에 남겨 뒤로 가기에 유지된다. 대시보드 출결의 미저장 입력이 있으면 이동 전에 확인한다.
- 관련 요구사항과 설계: [FR-15](./requirements.md#기능-요구사항) 화면 탐색 추가(KPI 정의 변경 없음). 사용자 요청·결정 2026-09-25(권장안: 타일 전체 클릭, 과제 완수율 → 통계, 대시보드 주소 유지). 검토 기록: 작업 기록 "대시보드 항목 링크 검토"
- 변경 대상: `pages/DashboardPage.tsx`, `pages/DailyPage.tsx`·`pages/StatsPage.tsx`(주소 값을 초기값으로), `components/ui/stat.tsx`(링크 타일), `lib/date.ts`(주 경계)
- 의존성: 없음
- 위험: 상단 메뉴로 이동할 때의 미저장 확인은 기존에도 없다(이번 범위 밖, `BrowserRouter`라 `useBlocker` 불가)
- 검증 방법: 선행 테스트 — 웹(링크 주소의 반·날짜·주 범위, 대시보드가 주소 값으로 열림, 미저장 확인 취소 시 머묾, 일일 입력·통계가 주소 값으로 조회), VER-35
- 완료 조건: 테스트 운영 배포

### TASK-72: 학생 목록 "반" 칸

- 상태: completed (2026-09-25 12:34)
- 상위: 없음
- 목표: 학생 탭 목록에 오늘 배정된 반 이름을 보여 준다(여러 반이면 모두, 비활성 반은 "(비활성)" 표시, 없으면 "—"). 탭 분리 뒤 학생 소속을 확인하는 용도.
- 관련 요구사항과 설계: [FR-05·FR-08](./requirements.md#기능-요구사항) 화면 표시(기능 변경 없음). 사용자 결정 2026-09-25(제안 ①)
- 변경 대상: `pages/StudentsPage.tsx`, 학생별 현재 반을 구하는 공용 훅(`lib/`) — 기존 `GET /classes?include_inactive=true` + 반마다 `GET /classes/{id}/enrollments?on=오늘` 조합. API 변경 없음(스코프는 서버의 반 목록 범위를 그대로 따른다)
- 의존성: 없음
- 위험: 반 수만큼 요청이 늘어난다(학원 규모 반 10여 개 수준이라 허용). 반이 많아지면 서버 집계 필드로 바꾼다
- 검증 방법: 선행 테스트 — 웹(활성·비활성·미배정 학생의 반 칸 표시), VER-35(390px 학생 표)
- 완료 조건: 반 칸 표시, 테스트 운영 배포

### TASK-73: 명단 다중 배정·반 옮기기 (FR-08)

- 상태: in-progress — 구현·배포 완료(2026-09-25 12:39), 사용자 확인 대기
- 상위: 없음
- 목표: 반 [명단]에서 학생 여러 명을 골라 한 번에 배정한다. "가져올 반"을 고르면 그 반 학생만 보이고, "옮기기"를 켜면 가져온 반의 배정을 시작일 전날로 끝낸다. 해제 종료일은 옮기기가 자동으로 정한다(TASK-71 보류분).
- 관련 요구사항과 설계: [FR-08](./requirements.md#기능-요구사항)(배정·해제 기간 이력) — 기존 `POST`·`PATCH` 배정 API를 학생마다 이어 부른다(새 배정 먼저, 성공하면 이전 반 종료 — 중간 실패 시 학생이 빠지는 대신 두 반에 남아 눈에 보인다). API 변경 없음. 사용자 결정 2026-09-25(제안 ②)
- 변경 대상: `pages/ClassesPage.tsx`의 `RosterDialog`
- 의존성: TASK-72(학생별 현재 반 훅)
- 위험: 학생마다 요청 1~2건(47명이면 최대 94건) — 순차 실행, 실패 학생은 이름·사유를 모아 보여 준다. 이전 배정이 새 시작일 당일·이후에 시작했으면 종료가 422 — 실패 목록에 나온다
- 검증 방법: 선행 테스트 — 웹(여러 명 선택 → 학생별 POST, 옮기기 켜면 이전 반 PATCH `end_date`=시작일 전날, 실패 학생 표시, 전체 선택)
- 완료 조건: 테스트 운영 배포, 사용자 확인

### TASK-68: 학생/반 관리 탭 분리

- 상태: completed (2026-09-25 00:35)
- 상위: 없음
- 목표: 상단 메뉴 "학생/반 관리" 안에 [학생]·[반] 하위 탭을 두고 하나씩 보여 준다. 기본은 학생 탭. 탭마다 주소(`/students`, `/students/classes` — 구현 시 확정, 작업 기록 참조)를 두어 새로 고침·뒤로 가기에도 탭이 유지되고, 두 주소 모두 상단 메뉴가 선택 상태로 보인다.
- 관련 요구사항과 설계: [FR-05·FR-07](./requirements.md#기능-요구사항) 화면 구성(기능 변경 없음). 사용자 요청·결정 2026-09-25
- 변경 대상: `App.tsx`(라우트), `components/AppShell.tsx`(메뉴 활성 판정), 하위 탭 컴포넌트, `ops/verify/responsive.py`(`/classes` 추가), `docs/design.md` 화면 구성 표기
- 의존성: 없음
- 위험: 기존 `/students` 링크·테스트가 학생·반을 한 화면으로 가정 — `Roster.test.tsx` 등 확인
- 검증 방법: 선행 테스트 — 웹(탭 전환·주소별 표시·메뉴 활성), VER-35
- 완료 조건: 두 탭이 주소로 나뉘어 보이고 테스트 운영 배포

### TASK-69: 빈 반 안내 (일일 입력)

- 상태: completed (2026-09-25 00:38)
- 상위: 없음
- 목표: 일일 수업 & 성적 입력에서 그 수업일에 배정된 학생이 없으면 "0 / 0명"만 보이지 않게, "이 반에 배정된 학생이 없습니다. 학생/반 관리 → 반 탭의 [명단]에서 배정하세요" 안내와 반 탭 링크를 보여 준다.
- 관련 요구사항과 설계: [FR-09](./requirements.md#기능-요구사항) 화면 표시. 계기: 2026-09-25 테스트 운영 "등원 현황 0/0명" 조사(새 반에 배정 0건)
- 변경 대상: `pages/DailyPage.tsx`
- 의존성: TASK-68(반 탭 주소)
- 위험: 배정은 있으나 시작일이 수업일 뒤인 경우도 같은 안내 — 문구에 "이 날짜에"를 넣어 구분
- 검증 방법: 선행 테스트 — 웹(학생 0명일 때 안내·링크, 있을 때 없음)
- 완료 조건: 안내 표시, 테스트 운영 배포

### TASK-70: 출결 잠금 알림 강화

- 상태: completed (2026-09-25 18:55) — 구현·배포 2026-09-25 00:46, 사용자 육안 확인 2026-09-25 18:55
- 상위: 없음
- 목표: 출결 확정(FR-09) 잠금을 눈에 띄게 한다. ① 표 바로 위 경고색 띠 "🔒 {날짜} 출결이 확정되어 잠겨 있습니다({확정 시각} 확정). 과제·테스트는 그대로 입력할 수 있습니다. [확정 해제]" ② 잠긴 출결 칸을 누르면 띠가 깜빡이며 강조되고 칸 옆에 짧은 안내 ③ 버튼 이름 [확인]/[편집] → [출결 확정]/[확정 해제], 출결 열 제목에 🔒. **확정한 사람 이름은 표시하지 않는다**(사용자 결정 2026-09-25 — 서버 변경 없음).
- 관련 요구사항과 설계: [FR-09](./requirements.md#기능-요구사항)(확정·편집 2단계 잠금 규칙 그대로). 계기: 2026-09-25 "출결 버튼이 눌리지 않음" 조사 — 고1 인A 9/25 수업이 확정 상태였고 잠금 표시가 표 아래 [편집] 버튼 이름뿐이었다
- 변경 대상: `pages/DailyPage.tsx`, `pages/DashboardPage.tsx`(같은 출결 표·확정 버튼 — 구현 시 추가), `components/AttendanceButtons.tsx`(잠긴 칸 클릭 피드백 — `disabled`는 클릭 이벤트가 없으므로 `aria-disabled` 방식 검토)
- 의존성: 없음
- 위험: 잠긴 칸을 클릭 가능하게 바꿀 때 실제 값이 바뀌지 않아야 한다(서버 409 규칙은 그대로)
- 검증 방법: 선행 테스트 — 웹(잠김 시 띠·해제 버튼·열 🔒, 잠긴 칸 클릭 시 값 불변·안내 표시, 버튼 이름), 잠김 화면 스크린샷
- 완료 조건: 테스트 운영 배포, 사용자 육안 확인

### TASK-71: 반 배정 시작일 선택 (FR-08)

- 상태: completed (2026-09-25 00:56)
- 상위: 없음
- 목표: 반 [명단]에서 배정할 때 시작일을 고른다(기본 오늘). 과거 날짜를 고르면 지난 수업일에도 그 학생이 나온다. [해제]의 종료일 선택도 함께 검토한다(작업 기록 "기능 공백: 수강 해제 종료일은 화면에서 항상 오늘").
- 관련 요구사항과 설계: [FR-08](./requirements.md#기능-요구사항)(배정 기간 이력 보존·과거 수업일 소속 반 재현) — API는 이미 시작·종료일을 받는다. 화면 미구현분이라 DCR 없음
- 변경 대상: `pages/ClassesPage.tsx`의 `RosterDialog`, `api.ts`
- 의존성: 없음(TASK-68 뒤면 반 탭 안에서 동작)
- 위험: 명단 목록은 "오늘" 기준 배정만 조회한다 — 미래 시작일 배정이 목록에서 사라져 보이지 않게 조회 기준 확인
- 검증 방법: 선행 테스트 — 웹(시작일 입력 → 요청 본문), 필요 시 API(과거 시작일 배정 후 그 날짜 일일 목록에 포함)
- 완료 조건: 과거 날짜 배정이 해당 수업일 일일 입력에 보이고, 테스트 운영 배포

### TASK-67: 강사의 담당 반 수정 (DCR-011)

- 상태: completed (2026-09-24 23:51)
- 상위: 없음
...
- 결과: `update_class`가 원장 전용 검사 대신 `_visible_class`(강사는 담당 반만, 아니면 403)를 쓰고, 강사는 본문의 `teacher_id`·`is_active`가 현재 값과 다르면 403·무변경, 저장 시 감사 로그 `class.update`(바뀐 칸 이름: name·grade·schedules). 웹 `ClassesPage`: 강사에게 반 등록 폼·[비활성]·수정 창의 활성 여부·담당 강사·삭제를 숨김(저장 본문은 현재 담당·활성 값을 그대로 보냄)

### TASK-64: 강사 계정 관리 화면 (FR-02)

- 상태: completed (2026-09-24 22:57)
- 상위: 없음
- 목표: 원장이 계정을 만들고(아이디·이름·역할·초기 비밀번호), 이름·역할을 고치고, 비활성화하는 화면을 만든다(API는 있고 화면이 없던 부분). **DCR-009로 확장:** 본인 비밀번호 변경, 원장의 비밀번호 재설정, 원장이 정한 비밀번호의 변경 강제(서버 거부 + 강제 화면), `app_user.must_change_password` 마이그레이션.
- 관련 요구사항과 설계: [FR-02·AC-39](./requirements.md#인수-조건), [DCR-009](./work/20260922-mathdesk-baseline/DCR-009-비밀번호-변경과-재설정.md)
- 변경 대상: 웹 학원 설정 또는 별도 화면, `api.ts`
- 의존성: 없음
- 위험: 본인 계정 비활성화·역할 강등으로 원장이 0명이 되는 경우 — 기존 API 규칙을 확인하고 없으면 화면·API에서 막는다
- 검증 방법: 선행 테스트 — 웹(목록·생성·수정·비활성화), API 규칙 보강 시 API 테스트
- 완료 조건: 원장이 화면에서 강사 계정을 만들고 그 계정으로 로그인할 수 있다. AC-39 통과(VER-39), 테스트 운영 배포
- 결과: 마이그레이션 `b7e4c1d2a9f0`(`app_user.must_change_password`, 기존 행 거짓), `POST /auth/password`(현재 비밀번호 확인·틀리면 로그인 실패 횟수에 포함·이 세션만 남김), `POST /users/{id}/password`(원장·같은 캠퍼스·본인 422·세션 전부 삭제·잠금 해제), 공통 스코프 검사의 변경 강제 403, `PATCH /users`의 **마지막 활성 원장 강등·비활성 409**(위험 항목에서 예고한 규칙 — 기존 API에 없었다). 웹: 학원 설정의 "계정 관리"(원장만, 목록·추가·수정·재설정), 상단 [비밀번호 변경], 변경 강제 화면

### TASK-65: 반 담당 강사 지정 (FR-07)

- 상태: completed (2026-09-24 23:04)
- 상위: 없음
- 목표: 반 등록·수정에서 담당 강사를 고른다(캠퍼스의 활성 강사 목록). API는 `teacher_id`를 받고 화면이 없던 승인된 요구사항이다.
- 관련 요구사항과 설계: [FR-07](./requirements.md#기능-요구사항)
- 변경 대상: `ClassesPage.tsx`, `api.ts`
- 의존성: TASK-64(선택지가 될 강사 계정)
- 위험: `PATCH /classes`는 전치환 — 강사 선택이 시간표·활성 여부를 지우지 않게 함께 보낸다. 다른 캠퍼스 사용자 ID를 넣지 못하게 서버 검증이 있는지 확인한다
- 검증 방법: 선행 테스트 — 웹(선택·저장 본문), API(타 캠퍼스·비강사 ID 거부가 없으면 보강)
- 완료 조건: 반에 강사를 지정하면 그 강사로 로그인했을 때 반 목록·일일 입력에 그 반이 보인다
- 결과: 서버 검증이 **없었다**(다른 캠퍼스 사용자·원장·없는 ID도 저장됨) → `masterdata._check_teacher` 추가: 새로 지정하는 담당은 같은 캠퍼스의 사용 중인 강사만(아니면 422), 이미 지정된 강사는 나중에 사용 안 함이 되어도 반을 고칠 때 그대로 허용. 웹: 원장에게만 반 등록 폼·수정 대화상자의 "담당 강사" 선택(지정 안 함 + 사용 중인 강사, 기존 담당이 사용 안 함이면 `(사용 안 함)` 표시로 유지), 반 목록 첫 줄에 강사 이름. 강사 화면은 `/users`를 부르지 않는다

### TASK-66: 주간 시간표 (DCR-008)

- 상태: completed (2026-09-24 23:19)
- 상위: 없음
...
- 검증 방법: 선행 테스트 — AC-38 ①~⑤, VER-35에 화면 추가
- 완료 조건: AC-38 통과, 테스트 운영 배포
- 결과: `timetable.py` — `GET /timetable`(기존 역할 스코프 `_visible_classes` 재사용, 활성 반만, 요일→시작 시각 순, `end_time`=시작+수업 길이·자정을 넘기면 다음 날 시각), `PUT /settings/class-minutes`(원장, 30~480, 캠퍼스별 `integration_setting.class_minutes`, 기본 120). 웹: 상단 탭 "주간 시간표"(`/timetable`) — 768px 이상은 요일 7열 × 시간 격자(1분=1px, 축은 가장 이른 시작~가장 늦은 끝, 겹치는 수업은 열 안에서 나란히), 좁은 화면은 요일별 목록, "시간표 미등록" 묶음, 원장만 강사 이름과 강사 선택(전체·강사별·담당 없음). 학원 설정에 "수업 길이" 카드(원장)

### TASK-63: 반 시간표 입력 (DCR-007)

- 상태: completed (2026-09-24 21:42)
- 상위: 없음
...
- 결과: 마이그레이션 `52183a89ef87`(`end_time` NULL 허용, downgrade는 빈 값을 시작 시각으로 채움), `ScheduleIn.end_time` 선택·요일+시작 중복 422, 웹 `ScheduleEditor`(요일 선택 + 시작 시각, [시간 추가]·[삭제])를 반 등록 폼과 수정 대화상자에 두고 표시는 `화 18:00 · 토 10:00`
...
| VER-40 | AC-40 | TASK-67 | 강사 담당 반 수정·거부 규칙·감사 로그 API 테스트 + 강사 화면 표시·수정 본문 웹 테스트 — **통과** (`test_class_teacher_edit.py` 6건, 웹 `ClassesPage.test.tsx` 강사 화면 3건) |
