...
| 학생·반 CRUD | 가능 | 담당 반 학생 조회·수정, 담당 반의 반명·학년·시간표 수정([DCR-011](./work/20260922-mathdesk-baseline/DCR-011-강사의-담당-반-수정.md)). 반 생성·삭제·담당 지정·활성 변경 불가 |
| 일일 기록 | 가능 | 담당 반만 가능 |
| 메시지 발송 | 가능 | 담당 반 학생만 가능 |
| 시험·OMR | 가능 | 담당 반 시험만 가능 |
| 사용자·연동 설정 | 가능 | 불가 |

#### DES-04 상세 — 학생·반 삭제 ([DCR-006](./work/20260922-mathdesk-baseline/DCR-006-퇴원-비활성-학생과-반의-삭제.md))

- 선행 조건: 학생 `퇴원`, 반 `비활성`. 아니면 `409`, 변경 없음. 원장만(강사 403), 캠퍼스 스코프는 공통 규칙.
- `GET …/deletion-preview`는 분류별 삭제 건수를 돌려준다. `DELETE`는 본문의 `confirm_name`이 대상 이름과 같아야 하며(다르면 `422`, 변경 없음) 성공 시 `204`.
- 한 트랜잭션에서 대상 행을 잠그고 선행 조건 확인 → 연관 데이터 삭제 → 대상 삭제를 한다. 실패하면 전부 되돌린다.
- 학생 삭제: `guardian`, `enrollment`, `student_daily_record`, `exam_answer`→`exam_attempt`, 매칭된 `omr_scan`과 그 `label_correction`(`source_ref=omr_scan:{id}:…`), `message_log`, `consult_log`. `stored_file`(OMR 원본 PDF)은 남긴다.
- 반 삭제: `class_schedule`, `enrollment`, 그 반 `class_session`의 `student_daily_record`·`class_session_progress`와 세션. `exam.class_id`는 `NULL`로 바꾸고 시험·응시는 남긴다.
- 감사 로그 `student.delete`·`class.delete`: 대상 ID와 분류별 건수. 이름·연락처는 쓰지 않는다.
- 새 테이블이 `student`·`class`를 참조하게 되면 삭제 규칙에 함께 분류해야 한다(모델 외래키를 읽는 테스트가 누락을 잡는다).
...
| `app_user` | `id`, `login_id`, `password_hash`, `display_name`, `role`, `is_active` | 역할 `director`·`teacher` `must_change_password`(원장이 정한 비밀번호면 참 — 참인 동안 `GET /auth/me`·로그아웃·`POST /auth/password` 외 요청은 공통 스코프 검사에서 403, [DCR-009](./work/20260922-mathdesk-baseline/DCR-009-비밀번호-변경과-재설정.md)) |
...
| 그룹 | 엔드포인트 |
|---|---|
| 인증 | `POST /auth/login`, `POST /auth/logout`, `GET /auth/me`, `POST /auth/password`(본인 변경, 현재 비밀번호 확인, 다른 세션 삭제) |
...
| 반·수강 | `GET/POST /classes`, `PATCH/DELETE /classes/{id}`(강사의 `PATCH`는 담당 반만, 본문의 `teacher_id`·`is_active`가 현재 값과 같아야 하며 아니면 403·무변경, 감사 로그 `class.update` — [DCR-011](./work/20260922-mathdesk-baseline/DCR-011-강사의-담당-반-수정.md)), `GET /classes/{id}/deletion-preview`, `GET/POST/DELETE /classes/{id}/enrollments` |
...
| 캐시 | `/api` 응답에는 `Cache-Control: no-store, private`를 붙인다. CDN이 인증된 응답을 엣지에 보관하면 URL만으로 열람할 수 있다. API 경로에 정적 파일 확장자를 쓰지 않는다 |
...
| RISK-10 | ~~헤드리스 브라우저 의존~~ 해소됨(2026-09-22). 이미지 라이브러리 직접 렌더로 전환해 브라우저 의존이 없다. 한글 글꼴 패키지만 이미지에 포함한다 | — | 해소됨 |
...
기준선 `v12` (2026-09-24): [DCR-011](./work/20260922-mathdesk-baseline/DCR-011-강사의-담당-반-수정.md) 재승인으로 강사가 담당 반의 반명·학년·시간표를 `PATCH /classes/{id}`로 수정할 수 있게 권한 매트릭스와 규칙이 바뀌었다(계약 형식 변경 없음).
...
| 2026-09-24 | 강사의 담당 반 수정 규칙, 권한 매트릭스 | [DCR-011](./work/20260922-mathdesk-baseline/DCR-011-강사의-담당-반-수정.md) | approved 유지, 기준선 v11 → v12 | Claude / 사용자 |
